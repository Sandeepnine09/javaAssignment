...
-------------------------------------------------------------------------------------------------------------------------------
	To add multiple sheets inside an Excel file along with "Sheet1" and populate them with the same data as "Sheet1," you need to make several changes to the code. Here's an updated version of the code that adds multiple sheets:
	...
	public class OpenXmlFormatter : IFormatter
{
    ...
    public Stream Execute(JObject graphQLResult, bool ignorePropertyPath)
    {
        ...
        if (list != null && list is JArray)
        {
            ...
            using (var excelStream = new MemoryStream())
            {
                ...
                // Create Sheet1 and populate with data
                var sheet1 = workbook.CreateSheet("Sheet1");
                PopulateSheet(graphQLResult, ignorePropertyPath, sheet1);

                // Create additional sheets and populate with the same data as Sheet1
                var sheetNames = new[] { "Sheet2", "Sheet3", "Sheet4" }; // Example additional sheet names
                foreach (var sheetName in sheetNames)
                {
                    var sheet = workbook.CreateSheet(sheetName);
                    PopulateSheet(graphQLResult, ignorePropertyPath, sheet);
                }
    ...
    private void PopulateSheet(JObject graphQLResult, bool ignorePropertyPath, ISheet sheet)
    {
        var header = (Dictionary<string, int>)null;
        var rowIndex = 0;
        var dictList = ((JArray)list).ToFlatDictionaries();
        foreach (var record in dictList)
        {
            if (rowIndex == 0)
            {
...
In this updated code, a loop is added to create additional sheets using the names specified in the sheetNames array. The PopulateSheet method is responsible for populating each sheet with the same data as "Sheet1."

Please note that you need to modify the sheetNames array to include the desired names for the additional sheets you want to create.
